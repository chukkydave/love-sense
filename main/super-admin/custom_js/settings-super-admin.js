...
function listAdmins(page) {
	...
	$.ajax({
		...
		dataType: 'json',
		cache: false,
		url: `${apiPaths}admin/view_super_admin/${page}/${page_limit}`,
		headers: {
		...
		success: (res) => {
			if (res.status == 200) {
				if (res.adminrecords.length > 0) {
					...
					$('#admin_body').html(
						`<tr class="even"> <td class=" " colspan="4">No super-admin record found</td></tr>`,
					);
					$('#adminModal').hide();
					$('#adminTable').show();
				}
...
function deleteMe(idex) {
	...
	if (idex != '') {
		...
		$.ajax({
			type: 'DELETE',
			dataType: 'json',
			url: `${apiPaths}admin/delete_super_admin/${idex}`,
			headers: {
				Accept: 'application/json',
				'Content-Type': 'application/json',
			...
			success: function(res) {
				if (res.status == 200) {
					alert('Super Admin delete successful');
					listAdmins(1);
				} else {
					alert('Error!!');
...
function createAdmin() {
	...
	$.ajax({
		...
		dataType: 'json',
		cache: false,
		url: `${apiPaths}super_admin/signup`,
		headers: {
			// 'Accept': 'application/json',
			'Content-Type': 'application/json',
			Authorization: token1,
